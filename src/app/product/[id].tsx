...
export default function ProductDetailScreen() {
  ...
  if (!product) {
    return (
      <View className={`flex-1 ${isDark ? 'bg-gray-900' : 'bg-white'} items-center justify-center p-6`}>
        <Feather name="alert-circle" size={48} color="#94A3B8" />
        <Text className={`${isDark ? 'text-gray-400' : 'text-gray-500'} mt-3 text-center`}>
          Product not found
        </Text>
        <TouchableOpacity 
      ...
      <View className={`flex-1 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
        <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
              ...
              >
                {wishlistLoading ? (
                  ...
                  <Feather 
                    name="heart" 
                    size={20} 
                    color={isWishlisted ? "#EF4444" : (isDark ? "#FFFFFF" : "#1F2937")}
          ...
          <View className="px-5 pt-6 pb-8">
            ...
            <View className={`h-px ${isDark ? 'bg-gray-800' : 'bg-gray-200'} my-5`} />
            ...
            <View className={`flex-row items-center ${isDark ? 'bg-gray-800' : 'bg-gray-50'} p-4 rounded-2xl`}>
              {product.sellerImage ? (
                <Image 
